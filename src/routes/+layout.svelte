<script lang="ts">
  ...
  import { characters, colorblindEnabled, picURI } from "$lib/store";
    ...
    <div class="relative flex w-full items-center justify-end gap-2">
      <Dialog.Root>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content
            class="motion-preset-fade-md bg-tertiary-500 fixed top-[50%] left-[50%] z-50 flex w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] flex-col items-center gap-4 rounded-md p-7 outline-hidden sm:max-w-lg md:w-full"
          >
        ...
        <Dialog.Portal>
          ...
          >
            ...
            <div class="guess-container grid grid-cols-5 grid-rows-1">
              {#each { length: 5 } as _, i}
                <div
                  class={`relative h-10 w-10 border border-black md:h-16 md:w-16 ${i === 2 ? "bg-missplace" : "bg-black"}`}
                >
                  <img
                    src={ReplacePlaceholders(picURI, { characterID: $characters![i + 5].id })}
                    alt={$characters![i + 5].name}
                  />
                </div>
              {/each}
            </div>
  ...
  {#snippet trail()}
    ...
    {:else}
      <div class="flex flex-nowrap gap-2">
        <LangSelector heigthClass="h-8"></LangSelector>
        <button class="settings flex items-center justify-center" onclick={openModalSettings}>
          <iconify-icon icon="uil:setting"></iconify-icon>
        </button>
        <button class="info flex items-center justify-center" onclick={openModalInfo}>
          <iconify-icon icon="uil:question-circle"></iconify-icon>
        </button>
      </div>
    {/if}
